...
random.seed(1234)
data_train, data_test, y_train, y_true, lemfpos_train, lemfpos_test = train_test_split(list_of_feature_vectors, list_of_g2p_annotations, list_of_lemfpos_and_sloleks_ids, test_size=0.2, stratify=list_of_g2p_annotations)

"""
# WRITE ALL DATASETS TO FILES
#print("DATA_TRAIN", data_train)
#print("y TRAIN", y_train)
output_with_data_train = open("data_train.tsv", "w", encoding="UTF-8")
for x in data_train:
    output_with_data_train.write("{}\n".format("\t".join([str(x1) for x1 in x])))
output_with_data_test = open("data_test,tsv", "w", encoding="UTF-8")
for x in data_test:
    output_with_data_test.write("{}\n".format("\t".join([str(x1) for x1 in x])))
output_with_y_train = open("labels_y_train.tsv", "w", encoding="UTF-8")
for x in y_train:
    output_with_y_train.write(f"{x}\n")
output_with_y_true = open("labels_y_test.tsv", "w", encoding="UTF-8")
for x in y_true:
    output_with_y_true.write(f"{x}\n")
output_with_lemfpos_train = open("lemfpos_train.tsv", "w", encoding="UTF-8")
for x in lemfpos_train:
    output_with_lemfpos_train.write(f"{x}\n")
output_with_lemfpos_test = open("lemfpos_test.tsv", "w", encoding="UTF-8")
for x in lemfpos_test:
    output_with_lemfpos_test.write(f"{x}\n")
"""
...
# TODO - PROBABILITIES!!!! Converting LinearSVC's decision function to probabilities (Scikit-learn Python)
...
output_with_crossvalidation_scores = open("crossvalidation_scores.txt", "w", encoding="UTF-8")
...
output_with_predictions = open("predictions.txt", "w", encoding="UTF-8")
...
def convert_list_of_strings_to_numbers(list_of_strings):
    dict_strings_and_their_numerical_values = dd()
    counter_for_numerical_values = 0
    for individual_element in sorted(list(set(list_of_strings))):
        counter_for_numerical_values += 1
        dict_strings_and_their_numerical_values[individual_element] = counter_for_numerical_values

    new_list = []
    for element in list_of_strings:
        new_list.append(dict_strings_and_their_numerical_values[element])

    return new_list


#y_true_as_vector_of_numeric_values = convert_list_of_strings_to_numbers(list_of_strings=y_true)
#y_test_as_vector_of_numeric_values = convert_list_of_strings_to_numbers(list_of_strings=y_test)


# https://scikit-learn.org/stable/modules/generated/sklearn.metrics.roc_auc_score.html#sklearn.metrics.roc_auc_score
# https://www.evidentlyai.com/classification-metrics/explain-roc-curve
# https://stackoverflow.com/questions/27357121/scikit-calculate-precision-and-recall-using-cross-val-score-function
#MICRO_roc_auc_score = metrics.roc_auc_score(y_true=y_true_as_vector_of_numeric_values, y_score=y_test_as_vector_of_numeric_values, average='micro')
#MACRO_roc_auc_score = metrics.roc_auc_score(y_true=y_true_as_vector_of_numeric_values, y_score=y_test_as_vector_of_numeric_values, average='macro')
#SAMPLES_roc_auc_score = metrics.roc_auc_score(y_true=y_true_as_vector_of_numeric_values, y_score=y_test_as_vector_of_numeric_values, average='samples')
#WEIGHTED_roc_auc_score = metrics.roc_auc_score(y_true=y_true_as_vector_of_numeric_values, y_score=y_test_as_vector_of_numeric_values, average='weighted')
#print("MICRO_ROC_AUC_score", MICRO_roc_auc_score)
#print("MACRO_ROC_AUC_score", MACRO_roc_auc_score)
#print("SAMPLES_ROC_AUC_score", SAMPLES_roc_auc_score)
#print("WEIGHTED_ROC_AUC_score", WEIGHTED_roc_auc_score)
...
print("|PRECISION|{}|".format(metrics.precision_score(y_true, y_test, pos_label="Other G2P")))
print("|RECALL|{}|".format(metrics.recall_score(y_true, y_test, pos_label="Other G2P")))
print("|F1-SCORE|{}|".format(metrics.f1_score(y_true, y_test, pos_label="Other G2P")))
...
output_with_crossvalidation_scores.write("|PRECISION: y_true vs. y_test; pos_label='Other G2P'|{}|\n".format(metrics.precision_score(y_true, y_test, pos_label="Other G2P")))
output_with_crossvalidation_scores.write("|RECALL: y_true vs. y_test; pos_label='Other G2P'|{}|\n".format(metrics.recall_score(y_true, y_test, pos_label="Other G2P")))
output_with_crossvalidation_scores.write("|F1-SCORE: y_true vs. y_test; pos_label='Other G2P'|{}|\n".format(metrics.f1_score(y_true, y_test, pos_label="Other G2P")))
...
# SAVE MODELS
pickle.dump(model, open('test_model_export.sav', 'wb'))
