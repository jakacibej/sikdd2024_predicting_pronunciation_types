...
unique_ngrams = [line.strip("\n").split("\t")[0] for line in open("unique_ngrams_ENTIRE_SLOLEKS3.tsv", "r", encoding="UTF-8").readlines()[1:] if int(line.strip("\n").split("\t")[1]) < 1000 and int(line.strip("\n").split("\t")[1]) >= 500]
...
for line in file_with_pronunciation_types[1:]:  # SKIP HEADERS
    ...
    if fpos in dict_fpos_to_float and pronunciation_type in ["Slovene G2P", "Slovene G2P with minor deviation", "Other G2P"]:
        ...
        features, feature_names = vectorize_lemfpos(lemma=lemma.lower(), fpos=fpos, list_of_unique_ngrams_for_vectorization=unique_ngrams, get_general_ngrams=are_you_getting_general_ngrams, get_ending_ngrams=are_you_getting_ending_ngrams, get_starting_ngrams=are_you_getting_starting_ngrams)
