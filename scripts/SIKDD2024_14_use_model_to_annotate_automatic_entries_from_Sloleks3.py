...
# LOAD MODEL
with open('./final_linearSVC_model/final_LinearSVC_model_export.pkcls', 'rb') as model:
    LinearSVC_model = pickle.load(model)

# INSTANTIATE VECTORIZER
vectorizer = Vectorizer(resources_directory="./ngram_lists/")

# OUTPUT FOR AUTOMATIC ENTRIES ANNOTATED WITH PRONUNCIATION TYPES
output = open("automatic_entries_with_predicted_pronunciation_types.tsv", "w", encoding="UTF-8")

# FILE WITH LEMFPOS
file_with_lemmas = open("lemfpos_and_pronunciation_type_MANUAL_CORRECTIONS.tsv", "r", encoding="UTF-8").readlines()
output.write("{}\n".format("\t".join([str(x) for x in ["sloleks_id", "lemma", "fpos", "pronunciation_type", "entry_status", "predicted_pronunciation_type"]])))

count_processed_automatic_entries = 0
for line in file_with_lemmas[1:]:  # SKIP HEADERS
    ...
    if entry_status == "AUTOMATIC":
        features, feature_names = vectorizer.vectorize_lemfpos(lemma=lemma, fpos=fpos)
        predicted_pronunciation_type = LinearSVC_model.predict([features])
        output.write("{}\n".format("\t".join([str(x) for x in [sloleks_id, lemma, fpos, pronunciation_type, entry_status, predicted_pronunciation_type]])))
        count_processed_automatic_entries += 1
        if count_processed_automatic_entries % 1000 == 0:
            print(f"{count_processed_automatic_entries} automatic entries processed.")
